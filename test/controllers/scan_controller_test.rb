...
class ScanControllerTest < ActionController::TestCase
  ...
  test "should fill in all fields" do
    ...
    post :check, params: { scan: { partner: format(users(:vtk)), item: '', count: '', options: 'out' } }
    assert_response :redirect
    assert_not_nil flash[:error]
    assert_equal flash[:error], "Please fill in all fields."

    post :check, params: { scan: { partner: '', item: format(items(:stoel)), count: '', options: 'out' } }
    assert_response :redirect
    assert_not_nil flash[:error]
    assert_equal flash[:error], "Please fill in all fields."
  end
  ...
  test "both params should exist" do
    post :check, params: { scan: { partner: format(users(:vtk)), item: 'b - 1', count: '1', options: 'out' } }
    assert_response :redirect
    assert_not_nil flash[:error]
    ...
    post :check, params: { scan: { partner: format(users(:vtk)), item: format(items(:stoel)), count: '1', options: 'out' } }
    assert_response :redirect
  ...
  test "checking out items from non existing reservations should show a force add" do
    post :check, params: { scan: { partner: format(users(:hilok)), item: format(items(:stoel)), count: '5', options: 'out' } }
    assert_response :success
  ...
  test "checking out too many items from existing reservations should show a force add" do
    post :check, params: { scan: { partner: format(users(:vtk)), item: format(items(:stoel)), count: '5', options: 'out' } }
    assert_response :success
