...
class ScanControllerTest < ActionController::TestCase
  ...
  test "should fill in all fields" do
    ...
    post :check, params: { scan: { partner: users(:vtk), item: '', count: '', options: 'out' } }
    assert_response :redirect
    assert_not_nil flash[:error]
    assert_equal flash[:error], "Please fill in all fields."
  ...
  test "both params should exist" do
    post :check, params: { scan: { partner: users(:vtk), item: 'b - 1', count: '1', options: 'out' } }
    assert_response :redirect
    assert_not_nil flash[:error]
    ...
    post :check, params: { scan: { partner: users(:vtk), item: items(:stoel), count: '1', options: 'out' } }
    assert_response :redirect
  ...
  test "checking in items from existing reservations should work" do
    post :check, params: { scan: { partner: users(:vtk), item: items(:stoel), count: '1', options: 'return_unused' } }
    assert_response :redirect
  ...
  test "checking in items from non existing reservations should display a warning" do
    post :check, params: { scan: { partner: users(:vlak), item: items(:stoel), count: '1', options: 'return_unused' } }
    assert_response :redirect
  ...
  test "checking out items from non existing reservations should show a force add" do
    post :check, params: { scan: { partner: users(:hilok), item: items(:stoel), count: '5', options: 'out' } }
    assert_response :success
  ...
  test "checking out too many items from existing reservations should show a force add" do
    post :check, params: { scan: { partner: users(:vtk), item: items(:stoel), count: '5', options: 'out' } }
    assert_response :success
    assert_select "strong", "already picked up"
  end

  test "should be able to force add items to existing reservations" do
    post :force, params: { force: { partner_id: users(:vtk), item_id: items(:stoel), count: 104 } }
    assert_response :redirect
    assert_not_nil flash[:notice]
  ...
  test "should be able to force add items by creating new reservations" do
    post :force, params: { force: { partner_id: users(:vtk), item_id: items(:tent), count: 100 } }
    assert_response :redirect
    assert_not_nil flash[:notice]
